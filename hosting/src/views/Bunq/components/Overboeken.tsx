...
const Overboeken = () => {
  ...
  const makePayment = (user: any) => async (state: any) => {
    ...
    if (state.internal.value) {
      body = {
        from: { type: 'description', value: state.fromAccount.value },
        to: { type: 'description', value: state.toAccount.value },
        ...
        <CardActions>
          <Button
            ...
            disabled={
              ...
              hasError ||
              submitting ||
              (state.internal.value && state.fromAccount.value === '') ||
